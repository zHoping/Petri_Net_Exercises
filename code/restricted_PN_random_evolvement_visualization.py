...
C_pre=pd.read_excel('data4.xls',sheet_name='C_pre',header=None) 
C_pre=np.array(C_pre)
C_post=pd.read_excel('data4.xls',sheet_name='C_post',header=None)
C_post=np.array(C_post)
M_0=pd.read_excel('data4.xls',sheet_name='M0',header=None)
M_0=np.array(M_0)
W_B=pd.read_excel('data4.xls',sheet_name='W_B',header=None)
W_B=np.array(W_B)
C=C_post-C_pre
P_num=C.shape[0]
T_num=C.shape[1]
W=W_B[:,0:P_num]
B=W_B[:,P_num:P_num+1]
C_post=np.r_[C_post,-np.dot(W,C)*(np.dot(W,C)>0)]
C_pre=np.r_[C_pre,np.dot(W,C)*(np.dot(W,C)>0)]
C=np.r_[C,-np.dot(W,C)]
M_0=np.r_[M_0,B-np.dot(W,M_0)]
M_current=M_0
print(M_current)
